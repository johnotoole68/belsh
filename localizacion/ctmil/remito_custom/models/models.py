				...
				for barrel in barrels_ids:
					...
					if barrel_data:
						if rec.partner_id.id == barrel_data[0].partner_id.id:
							...
							table = table + '<tr><td>' + str(barrel_data[0].origin) + '</td><td>' + inv_num + '</td><td>' + str(datetime.strptime(date_invoice, '%d/%m/%Y')) + '</td><td>' + str(barrel_data[0].picking_id.name) + '</td><td>' + str(nro_barril) + '</td><td></td><td> </td><td> </td></tr>'
